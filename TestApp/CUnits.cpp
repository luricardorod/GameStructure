	...
	for (auto &gameObject : m_World->m_pGameObjects) {
		if (gameObject->GetClassId() == CLASSUNIT)
		{
			...
			if (IsAnEnemyFaction(tempUnit->m_iIdFaction))
			{
				...
				if (distance < m_World->GetType(m_iIdType)->m_fVisionRange)
				{
...
{
	return m_World->GetType(m_iIdType)->m_bIsBelic;
}
...
{
	return unit->m_World->GetType(unit->m_iIdType)->m_iMaxGarrison < unit->m_vListaGarrison->size();
}
